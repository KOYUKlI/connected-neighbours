...
export function addDisputeEvidence(
  id: string,
  input: { message?: string; fileId?: string },
) {
...
export function uploadDisputeEvidenceFile(
  ...
  return uploadProofFile(
    `/api/disputes/${id}/evidence/presign-upload`,
    file,
    onPhase,
  );
}

export function getDisputeEvidenceDownloadUrl(
  disputeId: string,
  evidenceId: string,
...
export function deleteDisputeEvidenceAttachment(
  disputeId: string,
  evidenceId: string,
) {
  return apiRequest<DisputeEvidence>(
    `/api/disputes/${disputeId}/evidence/${evidenceId}/attachment`,
    { method: "DELETE" },
  );
}
